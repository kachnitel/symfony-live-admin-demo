...
class BundleAdminControllerTest extends WebTestCase
{
    ...
    public function testPartListRendersArchiveToggle(): void
    {
        $client = $this->createAuthenticatedClient();
        $crawler = $client->request('GET', '/admin/part');

        $this->assertResponseIsSuccessful();
        // The archive toggle button should appear when archiveExpression is configured
        // The bundle renders an "archived" filter / toggle control
        $pageSource = $client->getResponse()->getContent();
        $this->assertStringContainsString('archived', strtolower($pageSource ?? ''));
    }
    ...
    public function testHomepageShowsFeatures(): void
    {
        ...
        $this->assertSelectorTextContains('body', 'Archive');
    }
}
